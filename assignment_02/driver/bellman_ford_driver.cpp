...
// ------------------------------------------------------------
// Write Bellman-Ford actual output
// ------------------------------------------------------------

static void write_output(
    const fs::path& file,
    const Bellman_Ford& result,
    double execution_time)
{
    ofstream out(file);
    ...
    const AlgorithmConfig& config)
{
    ...
    Bellman_Ford output =
        bellman_ford_distance(csr_graph, source);
    ...
    /*
     * There is no separate expected-output file requirement
     * for the current driver structure.
     *
     * Therefore, once the algorithm completes successfully,
...
int bellman_ford_driver_main(
    int argc,
    char* argv[])
{
    if (argc < 2)
    {
        cerr << "Usage: bellman_ford_driver "
             << "<file ... | all>\n";
     ...
     * etc.
     *
     * We search upwards until assignmentConfig.dirname
     * is found.
     */
    ...
    // --------------------------------------------------------
    // Find Bellman-Ford configuration
    // --------------------------------------------------------
    ...
    {
        throw runtime_error(
            "Bellman-Ford configuration not found.");
    }
    ...
    readme << "| Test File | Vertices | Edges | Source | "
...
int main(int argc, char* argv[])
{
    try
    {
        return bellman_ford_driver_main(
            argc,
            argv);
    }
    catch (const exception& e)
