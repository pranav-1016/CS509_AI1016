...
//
// V
// matrix[0][0] matrix[0][1] ... matrix[0][V-1]
// ...
// matrix[V-1][0] ... matrix[V-1][V-1]
//
    ...
    CSRGraph& graph)
{
    ...
    // --------------------------------------------------------
    // Temporarily store the matrix as adjacency lists.
    // This is only input preparation and is NOT TIMED.
    // --------------------------------------------------------
    ...
    for (int i = 0; i < V; ++i)
    {
        for (int j = 0; j < V; ++j)
        {
            ...
            /*
             * A finite matrix entry represents a directed edge.
             *
             * We keep even a diagonal entry if the input
    ...
    graph.col_idx.reserve(edge_count);
    graph.values.reserve(edge_count);

    int position = 0;

    for (int i = 0; i < V; ++i)
    {
        graph.row_ptr[i] = position;
...
// ------------------------------------------------------------
// Write Floyd-Warshall actual output
// ------------------------------------------------------------

static void write_output(
    const fs::path& file,
    const Floyd_Warshall& result,
    double execution_time)
{
    ofstream out(file);
    ...
    else
    {
        ...
        for (size_t i = 0;
             i < result.distance.size();
             ++i)
        {
            for (size_t j = 0;
                 j < result.distance[i].size();
                 ++j)
            {
                if (result.distance[i][j] == INF)
                    out << "INF";
                else
                    out << result.distance[i][j];
    ...
    const AlgorithmConfig& config)
{
    ...
    result.vertices = csr_graph.V;
    result.edges = csr_graph.E;


    // --------------------------------------------------------
    // Floyd-Warshall
    //
    // ONLY algorithm execution is timed.
    // --------------------------------------------------------

    auto start =
        chrono::steady_clock::now();

    Floyd_Warshall output =
        floyd_warshall_distance(csr_graph);
    ...
    /*
     * Expected-output validation is intentionally skipped.
     *
     * A successfully executed test is considered PASS.
     */
...
// ------------------------------------------------------------
// Reusable driver entry point
// ------------------------------------------------------------

int floyd_warshall_driver_main(
    int argc,
    char* argv[])
{
    if (argc < 2)
    {
        cerr << "Usage: floyd_warshall_driver "
             << "<file ... | all>\n";
    ...
    while (true)
    {
        ...
        {
            throw runtime_error(
                "Could not find Assignment 02 root.");
        }
    ...
    // --------------------------------------------------------
    // Find Floyd-Warshall configuration
    // --------------------------------------------------------
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else
        {
            ...
            /*
             * If the user gives just:
             *
             * fw_10.txt
             *
             * look inside the configured test directory.
             */
...
#ifndef FLOYD_WARSHALL_DRIVER_NO_MAIN
    ...
    char* argv[])
{
    try
    {
        return floyd_warshall_driver_main(
            argc,
            argv);
    }
    catch (const exception& e)
